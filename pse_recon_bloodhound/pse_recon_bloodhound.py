...
init_parser = argparse.ArgumentParser(description='havoc playbook - PowerShell Empire Builtin Recon: Bloodhound')
...
module_config = dict(config.items('bloodhound'))
executing_module = 'powershell/situational_awareness/network/bloodhound'
    ...
    try:
        ...
        if module_results['outcome'] == 'success':
            for module_result in module_results['results']:
                if module_result['taskID'] == module_task_id and 'Invoke-BloodHound completed' in module_result['results']:
