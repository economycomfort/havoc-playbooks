...
init_parser = argparse.ArgumentParser(description='havoc playbook - PowerShell Empire Builtin Recon: Share Finder')
...
config.read('havoc-playbooks/pse_recon_share_finder/pse_recon_share_finder.ini')

c2_task_name = config.get('c2_task', 'task_name')
c2_agent_name = config.get('c2_task', 'agent_name')
module_config = dict(config.items('share_finder'))
executing_module = 'powershell/situational_awareness/network/powerview/share_finder'
    ...
    try:
        ...
        if module_results['outcome'] == 'success':
            for module_result in module_results['results']:
                if module_result['taskID'] == module_task_id and 'Find-DomainShare completed' in module_result['results']:
